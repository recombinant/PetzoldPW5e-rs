...
use winapi::shared::minwindef::{
    BOOL, DWORD, HIWORD, HMODULE, HRGN, LOWORD, LPARAM, LRESULT, MAKELONG, WORD, WPARAM,
};
use winapi::shared::ntdef::{LPCWSTR, LPWSTR};
use winapi::shared::windef::{
    HBITMAP, HBRUSH, HDC, HFONT, HGDIOBJ, HPALETTE, HPEN, HWND, LPRECT, RECT,
};
use winapi::um::wingdi::{
    CombineRgn, DeleteObject, GetStockObject, SelectObject, RGN_AND, RGN_COPY, RGN_DIFF, RGN_OR,
    RGN_XOR,
};
use winapi::um::winnls::LCTYPE;
use winapi::um::winuser::{EnableWindow, GetWindowLongPtrW, InflateRect, SendMessageW};

use winapi::um::winuser::{
    LB_ADDSTRING, LB_DELETESTRING, LB_DIR, LB_FINDSTRING, LB_FINDSTRINGEXACT, LB_GETCARETINDEX,
    LB_GETCOUNT, LB_GETCURSEL, LB_GETHORIZONTALEXTENT, LB_GETITEMDATA, LB_GETITEMHEIGHT,
    LB_GETITEMRECT, LB_GETSEL, LB_GETSELCOUNT, LB_GETSELITEMS, LB_GETTEXT, LB_GETTEXTLEN,
    LB_GETTOPINDEX, LB_INSERTSTRING, LB_RESETCONTENT, LB_SELECTSTRING, LB_SELITEMRANGE,
    LB_SETCARETINDEX, LB_SETCOLUMNWIDTH, LB_SETCURSEL, LB_SETHORIZONTALEXTENT, LB_SETITEMDATA,
    LB_SETITEMHEIGHT, LB_SETSEL, LB_SETTABSTOPS, LB_SETTOPINDEX,
};
...
pub fn ListBox_SetColumnWidth(hwndCtl: HWND, cxColumn: c_int) {
    unsafe {
        SendMessageW(hwndCtl, LB_SETCOLUMNWIDTH, cxColumn as WPARAM, 0);
    }
}
...
pub fn ListBox_SetHorizontalExtent(hwndCtl: HWND, cxExtent: c_int) {
    unsafe {
        SendMessageW(hwndCtl, LB_SETHORIZONTALEXTENT, cxExtent as WPARAM, 0);
    }
}
...
pub fn ListBox_Dir(hwndCtl: HWND, attrs: UINT, lpszFileSpec: LPCWSTR) -> c_int {
    ...
    unsafe {
        SendMessageW(hwndCtl, LB_DIR, attrs as WPARAM, lpszFileSpec as LPARAM) as DWORD as c_int
    }
}
